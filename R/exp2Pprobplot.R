...
probplot.exp2P <- function(data,pp="Blom",xlabel1="X",confid=0.95,
  ...
  if(is.null(MLE_i)==TRUE){ # Switch off for LSQ
    best_fit_label <- "2P-Exponential (LSQ) Best-Fit"
  }
  if(is.null(MLE_i)==FALSE){ # Switch on for MLE
    best_fit_label <- "2P-Exponential (MLE) Best-Fit"
  }
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      if(is.null(MLE_i) == FALSE){  # MLE for all parameters θ and σ
        # Compute MLE
      ...
      if(is.null(MLE_i) == TRUE){
        ...
        outputpp1[[i]][[3]]<-matrix(unlist(ttfc_MLE_list[[i]][[3]]), nrow = 2, ncol = 2, byrow = FALSE,dimnames = list(c("Lower CI","Upper CI"),c("theta", "sigma"))) # Confidence
  ...
  if(is.null(MLE_i) == TRUE){
    ...
    theta_set <- rep(0,length(databystress))
    ...
    for(i in 1:length(databystress)){
      ...
      theta_set[i] <- outputpp1[[i]][[2]][1]
      sigma_set[i] <- outputpp1[[i]][[2]][2]
      R2_set[i] <- outputpp1[[i]][[3]]
    }
    matset <- c(stress_set,theta_set,sigma_set,SSEtot,R2_set)

    # Produce some output text that summarizes the results
    cat(c("Two Parameter Exponential Least-Squares estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
      print(matrix(matset, nrow = length(databystress), ncol = 5, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress","\U03B8","\U03C3","SSE","R\U00B2"))))
    }
    if(length(3:length(databystress[[i]][1,])) == 2){
      print(matrix(matset, nrow = length(databystress), ncol = 6, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress1","Stress2","\U03B8","\U03C3","SSE","R\U00B2"))))
    }
    cat("\n")
  } else{
    # Logging
    stress_set <- rep(0,length(databystress)*length(3:length(databystress[[i]][1,])))
    theta_set <- rep(0,length(databystress))
    theta_setCIlow <- rep(0,length(databystress))
    theta_setCIhigh <- rep(0,length(databystress))
    sigma_set <- rep(0,length(databystress))
    ...
    for(i in 1:length(databystress)){
      ...
      theta_set[i] <- outputpp1[[i]][[2]][1]
      theta_setCIlow[i] <- ttfc_MLE_list[[i]][[3]][[1]][1]
      theta_setCIhigh[i] <- ttfc_MLE_list[[i]][[3]][[1]][2]
      sigma_set[i] <- outputpp1[[i]][[2]][2]
    ...
    # Produce some output text that summarizes the results
    cat(c("Two Parameter Exponential Maximum Likelihood estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
      matset <- c(stress_set,theta_set,theta_setCIlow,theta_setCIhigh,sigma_set,sigma_setCIlow,sigma_setCIhigh,loglik_set,AIC_set)
      print(matrix(matset, nrow = length(databystress), ncol = 9, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress","\U03B8","\U03B8 Lower CI","\U03B8 Upper CI","\U03C3_t","\U03C3 Lower CI","\U03C3 Upper CI","loglik","AIC"))))
    }
    if(length(3:length(databystress[[i]][1,])) == 2){
      matset <- c(stress_set,theta_set,theta_setCIlow,theta_setCIhigh,sigma_set,sigma_setCIlow,sigma_setCIhigh,loglik_set,AIC_set)
      print(matrix(matset, nrow = length(databystress), ncol = 10, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress1","Stress2","\U03B8","\U03B8 Lower CI","\U03B8 Upper CI","\U03C3","\U03C3_t Lower CI","\U03C3 Upper CI","loglik","AIC"))))
    }
    cat("\n")
  }
