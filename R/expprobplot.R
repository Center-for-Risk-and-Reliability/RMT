...
probplot.exp <- function(data,pp="Blom",xlabel1="X",confid=0.95,
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      if(is.null(MLE_i) == FALSE){  # MLE for λ
        ...
        ttfc_list[[i]][[1]] <- -log(1-fcB)/ttfc_MLE_list[[i]][[1]][1]
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      xlines[((i*length(Pticks1)) - (length(Pticks1)-1)):(i*length(Pticks1))] <- -log(1-Pticks)/ttfc_list[[i]][[3]][1]
      ...
      line_legend[((i*length(Pticks)) - (length(Pticks1)-1)):(i*length(Pticks))] <- rep(paste(c("\U03BB = ",outputpp1[[i]][[2]],2),collapse = ""),length(Pticks))
      if(length(databystress) > 1){ # multiple groups of data
