...
probplot.logn <- function(data,pp="Blom",xlabel1="X",confid=0.95,
                          nobounds = NULL,stpstr_i=NULL,MLE_i=NULL,setsigma=NULL,CDFdata = NULL,CDFrangesetting = 1,
  ...
  if(is.null(MLE_i)==TRUE){ # Switch off for LSQ
    best_fit_label <- "Lognormal (LSQ) Best-Fit"
  }
  if(is.null(MLE_i)==FALSE){ # Switch on for MLE
    best_fit_label <- "Lognormal (MLE) Best-Fit"
  }
  ...
  if (!is.null(dim(databystress))){
    ...
    ttfc <- probplotparam.logn(xiRFblock[,1],xiRFblock[,2],CDFrangesetting)
    ttfcrange <- ttfc[[1]]
    params<-ttfc[[3]]
    ...
    for(i in 1:length(databystress)){
      ...
      ttfc_list[[i]]<-probplotparam.logn(xiRFblock_list[[i]][,1],xiRFblock_list[[i]][,2],CDFrangesetting)

      # CASE 1: Both μ_t and σ_t unknown
      if(is.null(MLE_i) == TRUE && is.null(setsigma) == TRUE){  # Default for LSQ
        outputpp[[i*3-1]]<-ttfc_list[[i]][[3]] # LSQ parameters
        outputpp1[[i]][[2]]<-ttfc_list[[i]][[3]] # LSQ parameters
      }
      if(is.null(MLE_i) == FALSE && is.null(setsigma) == TRUE){  # MLE for all parameters μ_t and σ_t
        # Compute MLE
        ...
        ttfc_list[[i]][[2]] <- fcB
        # if(length(xiRFblock_list[[i]][,1]) == 1){
        #   ttfc_MLE_list[[i]] <- ttfc_list[[i]][3]
        # }
        outputpp[[i*3-1]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 2, byrow = TRUE,dimnames = list(c("Lognormal Parameters"),c("mu_t", "sigma_T")))
        outputpp1[[i]][[2]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 2, byrow = TRUE,dimnames = list(c("Lognormal Parameters"),c("mu_t", "sigma_t")))
      ...
      if(is.null(MLE_i) == TRUE){
        ...
        outputpp1[[i]][[3]]<-matrix(unlist(ttfc_MLE_list[[i]][[3]]), nrow = 2, ncol = 2, byrow = FALSE,dimnames = list(c("Lower CI","Upper CI"),c("mu_t", "sigma_t"))) # Confidence
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      line_legend[((i*2) - 1):(i*2)] <- rep(paste(c("\U03BC_t = ",num2str(outputpp1[[i]][[2]][1],2),", \U03C3_t = ",num2str(outputpp1[[i]][[2]][2],2)),collapse = ""),2)
      if(length(databystress) > 1){ # multiple groups of data
        line_breaks[i] <- paste(c("\U03BC_t = ",num2str(outputpp1[[i]][[2]][1],2),", \U03C3_t = ",num2str(outputpp1[[i]][[2]][2],2)),collapse = "")
      } else{ # only one group
        line_breaks <- waiver()
      }
    }
    ...
    # plotout <- plotout + geom_line(data=df2, aes(Xline,Fline, linetype = best_fit), linewidth = 0.4, colour = 'black')
    ...
    if(is.null(nobounds) == TRUE){
      plotout <- plotout + geom_ribbon(data=df3, aes(x=Xline2up, ymin=Fline2up, ymax=Fline2down, fill = conf_legend), alpha=0.1) +
  ...
  if(is.null(MLE_i) == TRUE){
    ...
    mu_set <- rep(0,length(databystress))
    sigma_set <- rep(0,length(databystress))
    ...
    for(i in 1:length(databystress)){
      ...
      mu_set[i] <- outputpp1[[i]][[2]][1]
      sigma_set[i] <- outputpp1[[i]][[2]][2]
      R2_set[i] <- outputpp1[[i]][[3]]
    }
    matset <- c(stress_set,mu_set,sigma_set,SSEtot,R2_set)

    # Produce some output text that summarizes the results
    cat(c("Lognormal Least-Squares estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
      print(matrix(matset, nrow = length(databystress), ncol = 5, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress","\U03BC_t","\U03C3_t","SSE","R\U00B2"))))
    }
    if(length(3:length(databystress[[i]][1,])) == 2){
      print(matrix(matset, nrow = length(databystress), ncol = 6, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress1","Stress2","\U03BC_t","\U03C3_t","SSE","R\U00B2"))))
    }
    cat("\n")
  } else{
    # Logging
    stress_set <- rep(0,length(databystress)*length(3:length(databystress[[i]][1,])))
    mu_set <- rep(0,length(databystress))
    mu_setCIlow <- rep(0,length(databystress))
    mu_setCIhigh <- rep(0,length(databystress))
    sigma_set <- rep(0,length(databystress))
    sigma_setCIlow <- rep(0,length(databystress))
    sigma_setCIhigh <- rep(0,length(databystress))
    ...
    for(i in 1:length(databystress)){
      ...
      mu_set[i] <- outputpp1[[i]][[2]][1]
      mu_setCIlow[i] <- ttfc_MLE_list[[i]][[3]][[1]][1]
      mu_setCIhigh[i] <- ttfc_MLE_list[[i]][[3]][[1]][2]
      sigma_set[i] <- outputpp1[[i]][[2]][2]
      sigma_setCIlow[i] <- ttfc_MLE_list[[i]][[3]][[2]][1]
      sigma_setCIhigh[i] <- ttfc_MLE_list[[i]][[3]][[2]][2]
      loglik_set[i] <- ttfc_MLE_list[[i]][[4]]
      AIC_set[i] <- ttfc_MLE_list[[i]][[6]]
    }

    # Produce some output text that summarizes the results
    cat(c("Lognormal Maximum Likelihood estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
      matset <- c(stress_set,mu_set,mu_setCIlow,mu_setCIhigh,sigma_set,sigma_setCIlow,sigma_setCIhigh,loglik_set,AIC_set)
      print(matrix(matset, nrow = length(databystress), ncol = 9, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress","\U03BC_t","\U03BC_t Lower CI","\U03BC_t Upper CI","\U03C3_t","\U03C3_t Lower CI","\U03C3_t Upper CI","loglik","AIC"))))
    }
    if(length(3:length(databystress[[i]][1,])) == 2){
      matset <- c(stress_set,mu_set,mu_setCIlow,mu_setCIhigh,sigma_set,sigma_setCIlow,sigma_setCIhigh,loglik_set,AIC_set)
      print(matrix(matset, nrow = length(databystress), ncol = 10, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress1","Stress2","\U03BC_t","\U03BC_t Lower CI","\U03BC_t Upper CI","\U03C3_t","\U03C3_t Lower CI","\U03C3_t Upper CI","loglik","AIC"))))
    }
    cat("\n")
  }
