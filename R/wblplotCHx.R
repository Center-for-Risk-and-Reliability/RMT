...
plot.C.hazard.wbl <- function(xi, rc = NULL, pp = "Blom", confid = 0.95, sided = "twosided", xlabel1 = "X"){
  library(pracma)
  library(mvtnorm)
  library(ggplot2)
  ...
  yHx <- -log(1 - pweibull(xrange, params$wblparams[2], params$wblparams[1]))
  ...
  # Form data frame
  df <- data.frame(X = xrange, YCHaz = yHx, YCHazlow = yHx_low, YCHazhigh = yHx_high, best_fit = rep("Fitted",1000))
  # return(list(df))
  df2 <- data.frame(Xpts = x_R[,1], Ypts = x_R[,5], nonparametric_data = rep("Failure Data",length(x_R[,1])))
  plotout<-ggplot() +
    geom_line(data=df, aes(X,YCHaz), colour = 'blue', size = 0.9, linetype = "dashed") +
    xlab(xlabel1) +
    ylab("Cumulative Hazard")
  plotout <- plotout + geom_ribbon(data=df, aes(ymin=YCHazlow, ymax=YCHazhigh, x=X), alpha=0.5, fill = "blue")
  plotout <- plotout + geom_point(data=df2, aes(Xpts,Ypts, shape = nonparametric_data), colour = 'red', size = 2.2)
  return(list(wbloutput = params,CumulativeHazardplot = plotout))
}
