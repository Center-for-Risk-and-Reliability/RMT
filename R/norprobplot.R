...
probplot.nor <- function(data,pp="Blom",xlabel1="X",confid=0.95,
  ...
  if(is.null(MLE_i)==TRUE){ # Switch off for LSQ
    best_fit_label <- "Normal (LSQ) Best-Fit"
  }
  if(is.null(MLE_i)==FALSE){ # Switch on for MLE
    best_fit_label <- "Normal (MLE) Best-Fit"
  }
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      ttfc_list[[i]]<-probplotparam.nor(xiRFblock_list[[i]][,1],xiRFblock_list[[i]][,2],CDFrangesetting)
      ...
      if(is.null(MLE_i) == FALSE){ # MLE for all parameters μ and σ
        ...
        ttfc_list[[i]][[2]] <- fcB
        outputpp[[i*3-1]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 2, byrow = TRUE,dimnames = list(c("Normal Parameters"),c("mu", "sigma")))
        outputpp1[[i]][[2]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 2, byrow = TRUE,dimnames = list(c("Normal Parameters"),c("mu", "sigma")))
  ...
  # Calculate the upper and lower bounds (RCS 2/4/2025)
  ...
  if(is.null(MLE_i) == TRUE){
    ...
    cat(c("Normal Least-Squares estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
    ...
    # Produce some output text that summarizes the results
    cat(c("Normal Maximum Likelihood estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
