...
probplot.wbl3P <- function(data,pp,xlabel1="X",confid=0.95,stpstr_i=NULL,MLE_i=NULL) {
  ...
  if (!is.null(dim(databystress))){
    ...
    ttfcrange <- ttfc[[1]]
    params<-ttfc[[3]]
    Xline0 <- c(params)[3] + exp(log(c(params)[1]) + (Pticks/c(params)[2]))
    ...
    for(i in 1:length(databystress)){
      ...
      ttfcrange<-c(ttfcrange,ttfc_list[[i]][[1]],ttfc_list[[i]][[3]][3])
    }
  ...
  if(min(ttfcrange) < 0){
    ...
    signs1 <- c(log10(1):ceiling(log10(max(ttfcrange))))
  } else{
  ...
  if (!is.null(dim(databystress))){
    ...
    df2 <- data.frame(Xline = Xline0, Fline = Pticks, best_fit = rep("Best-fit",length(Pticks)))
