# Grigorten Non-Parametric Plot
# Developed by Dr. Reuel Smith, 2020-2022

nonparamplot.grigorten <- function(xi, rc, relfcn, confbnd, xlabel) {
  if(missing(relfcn)) {
    relfcn<-"reliability"
  }
  ...
  if (missing(rc)){
    FRhH<-plotposit.grigorten(rankcalc(xi),xi)
    nonparamset<-plottable.nonparam(xi, NULL, FRhH, relfcn, alpha, xlabel)
  } else {
    FRhH<-plotposit.grigorten(rankcalc(xi,rc),xi, rc)
