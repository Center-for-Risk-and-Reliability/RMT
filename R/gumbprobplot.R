...
# Developed by Dr. Reuel Smith, 2023-2025

probplot.gumb <- function(data,pp="Blom",xlabel1="X",confid=0.95,
  ...
  if(is.null(MLE_i)==TRUE){ # Switch off for LSQ
    best_fit_label <- "Gumbel (LSQ) Best-Fit"
  }
  if(is.null(MLE_i)==FALSE){ # Switch on for MLE
    best_fit_label <- "Gumbel (MLE) Best-Fit"
  }
  ...
  if (!is.null(dim(databystress))){
    ...
    # Data pull
    XB<-xiRFblock[,1]
    FB <- log(-log(xiRFblock[,3]))
    ttfc <- probplotparam.gumb(xiRFblock[,1],xiRFblock[,3])
    ttfcrange <- ttfc[[1]]
    params<-ttfc[[3]]
    ...
    for(i in 1:length(databystress)){
      ...
      FB_list[[i]] <- log(-log(xiRFblock_list[[i]][,3]))
      ttfc_list[[i]]<-probplotparam.gumb(xiRFblock_list[[i]][,1],xiRFblock_list[[i]][,3],CDFrangesetting)
      ...
      if(is.null(MLE_i) == FALSE){ # MLE for all parameters μ and σ
        # Compute MLE
        # CHECK THIS
        ttfc_MLE_list[[i]] <- distribution.MLEest(c(ttfc_list[[i]][[3]]),"Gumbel",ixirc_list[[i]][[2]],ixirc_list[[i]][[3]])
        # Recalculate x-points if MLE
        ttfc_list[[i]][[1]] <- c(ttfc_MLE_list[[i]][[1]][1]) + c(ttfc_MLE_list[[i]][[1]][2])*fcB

        outputpp[[i*3-1]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 2, byrow = TRUE,dimnames = list(c("Gumbel Parameters"),c("mu", "sigma")))
        outputpp1[[i]][[2]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 2, byrow = TRUE,dimnames = list(c("Gumbel Parameters"),c("mu", "sigma")))
      ...
      if(is.null(MLE_i) == TRUE){
        ...
        outputpp1[[i]][[3]]<-matrix(unlist(ttfc_MLE_list[[i]][[3]]), nrow = 2, ncol = 2, byrow = FALSE,dimnames = list(c("Lower CI","Upper CI"),c("mu", "sigma"))) # Confidence
  ...
  for(i in 1:length(databystress)){
    ...
    FB_bound <- log(-log(1-Fbound))
    FB_high <- log(-log(1-F_high))
    FB_low <- log(-log(1-F_low))

    Xbound_list[[i]] <- c(ttfc_list[[i]][[3]][1]) + c(ttfc_list[[i]][[3]][2])*FB_bound
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      line_legend[((i*2) - 1):(i*2)] <- rep(paste(c("\U03BC = ",num2str(outputpp1[[i]][[2]][1],2),", \U03C3 = ",num2str(outputpp1[[i]][[2]][2],2)),collapse = ""),2)
      if(length(databystress) > 1){ # multiple groups of data
        line_breaks[i] <- paste(c("\U03BC = ",num2str(outputpp1[[i]][[2]][1],2),", \U03C3 = ",num2str(outputpp1[[i]][[2]][2],2)),collapse = "")
      } else{ # only one group
        line_breaks <- waiver()
      }
    }
  ...
  if(is.null(MLE_i) == TRUE){
    ...
    cat(c("Gumbel Least-Squares estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
      print(matrix(matset, nrow = length(databystress), ncol = 5, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress","\U03BC","\U03C3","SSE","R\U00B2"))))
    }
    if(length(3:length(databystress[[i]][1,])) == 2){
      print(matrix(matset, nrow = length(databystress), ncol = 6, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress1","Stress2","\U03BC","\U03C3","SSE","R\U00B2"))))
    }
    cat("\n")
  } else{
    # Logging
    ...
    # Produce some output text that summarizes the results
    cat(c("Gumbel Maximum Likelihood estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
      matset <- c(stress_set,mu_set,mu_setCIlow,mu_setCIhigh,sigma_set,sigma_setCIlow,sigma_setCIhigh,loglik_set,AIC_set)
      print(matrix(matset, nrow = length(databystress), ncol = 9, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress","\U03BC","\U03BC Lower CI","\U03BC Upper CI","\U03C3","\U03C3 Lower CI","\U03C3 Upper CI","loglik","AIC"))))
    }
    if(length(3:length(databystress[[i]][1,])) == 2){
      matset <- c(stress_set,mu_set,mu_setCIlow,mu_setCIhigh,sigma_set,sigma_setCIlow,sigma_setCIhigh,loglik_set,AIC_set)
      print(matrix(matset, nrow = length(databystress), ncol = 10, byrow = FALSE,dimnames = list(rep("",length(databystress)),c("Stress1","Stress2","\U03BC","\U03BC Lower CI","\U03BC Upper CI","\U03C3","\U03C3 Lower CI","\U03C3 Upper CI","loglik","AIC"))))
    }
    cat("\n")
  }
