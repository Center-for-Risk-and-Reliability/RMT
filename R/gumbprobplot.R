...
# Developed by Dr. Reuel Smith, 2023-2024

probplot.gumb <- function(data,pp,xlabel1="X",confid=0.95,stpstr_i=NULL,MLE_i=NULL) {
  library(ggplot2)
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      ttfc_list[[i]]<-probplotparam.gumb(xiRFblock_list[[i]][,1],xiRFblock_list[[i]][,3])
