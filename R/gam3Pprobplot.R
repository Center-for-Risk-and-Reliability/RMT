...
probplot.gam3P <- function(data,pp,xlabel1="X",confid=0.95,stpstr_i=NULL,MLE_i=NULL) {
  library(pracma)
  library(zipfR)
  library(ggplot2)
  ...
  if (is.list(databystress) && length(databystress)==1){
    ...
    ttfc <- probplotparam.gam3P(xiRFblock[,1],xiRFblock[,2])
    ttfcrange <- ttfc[[1]]
    params<-ttfc[[3]]
    ...
    fcB <- Rgamma.inv(params[1],c(0.001,1-0.999),lower = FALSE)
  } else {
    ...
    for(i in 1:length(databystress)){
      ...
      ttfc_list[[i]]<-probplotparam.gam3P(xiRFblock_list[[i]][,1],xiRFblock_list[[i]][,2])
  ...
  fcB <- Rgamma.inv(alphaavg,c(.001,0.999),lower = TRUE)
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      Flines[((i*2) - 1):(i*2)] <- Rgamma.inv(alphaavg,c(0.001,0.999),lower = TRUE)
