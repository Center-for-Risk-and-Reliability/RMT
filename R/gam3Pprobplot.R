# Generalized Three-Parameter Gamma Probability Plot
# Developed by Dr. Reuel Smith, 2024-2025

probplot.gam3P <- function(data,pp="Blom",xlabel1="X",confid=0.95,
  ...
  if(is.null(MLE_i)==TRUE){ # Switch off for LSQ
    best_fit_label <- "Generalized Gamma (LSQ) Best-Fit"
  }
  if(is.null(MLE_i)==FALSE){ # Switch on for MLE
    best_fit_label <- "Generalized Gamma (MLE) Best-Fit"
  }
  ...
  # if (is.list(databystress) && length(databystress)==1){
    ...
    Pticks_list <- vector(mode = "list", length = 3*length(databystress)+1)
    SSEtot<-rep(1, length(databystress))
    ttfcrange<-c(1)
    XBfull<-c(1)
    FBfull<-c(1)
    # Catalog the shape parameters to get an average
    alphatot<-rep(1, length(databystress))
    betatot<-rep(1, length(databystress))
    gammatot<-rep(1, length(databystress))
    # Multi-stress data
    for(i in 1:length(databystress)){
      ...
      ttfc_list[[i]]<-probplotparam.gam3P(xiRFblock_list[[i]][,1],xiRFblock_list[[i]][,2],CDFrangesetting)
      ...
      if(is.null(MLE_i) == FALSE){ # MLE for all parameters
        # Compute MLE
        ttfc_MLE_list[[i]] <- distribution.MLEest(c(ttfc_list[[i]][[3]]),"3PGamma",ixirc_list[[i]][[2]],ixirc_list[[i]][[3]])
        # Recalculate x-points if MLE
        outputpp[[i*3-1]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 3, byrow = TRUE,dimnames = list(c("Generalized Gamma Parameters"),c("alpha", "beta","gamma"))) # MLE parameters
        outputpp1[[i]][[2]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 3, byrow = TRUE,dimnames = list(c("Generalized Gamma Parameters"),c("alpha", "beta","gamma"))) # MLE parameters
      ...
      alphatot[i]<-ttfc_list[[i]][[3]][1]
      gammatot[i]<-ttfc_list[[i]][[3]][3]
      XBfull<-c(XBfull,XB_list[[i]])
      FBfull<-c(FBfull,FB_list[[i]])
      ttfcrange<-c(ttfcrange,ttfc_list[[i]][[1]])
    }
    # return(alphatot)
    # RCS06272024 - Average the gamma shape parameters to get one to scale for the plot
    alphaavg <- mean(alphatot)
    gammaavg <- mean(gammatot)
    for(i in 1:length(databystress)){
      FB_list[[i]] <- Rgamma.inv(alphaavg,xiRFblock_list[[i]][,2],lower = TRUE)
      fitbetagamma_parm  <- lm(log(FB_list[[i]]) ~ poly(log(xiRFblock_list[[i]][,1]), 1, raw=TRUE))
      gammatot[i] <- summary(fitbetagamma_parm)$coefficients[2,1]
      betatot[i] <- exp(-summary(fitbetagamma_parm)$coefficients[1,1]/gammatot[i])
      FBfull<-c(FBfull,FB_list[[i]])
    }
  ...
  Pticks <- Rgamma.inv(alphaavg,Pticks1/100,lower = TRUE)
  fcB <- Rgamma.inv(alphaavg,c(Fmin,Fmax),lower = TRUE)
  ...
  for(i in 1:length(databystress)){
    ...
    FB_bound <- Rgamma.inv(alphaavg,Fbound,lower = TRUE)
    FB_high <- Rgamma.inv(alphaavg,F_high,lower = TRUE)
    FB_low <- Rgamma.inv(alphaavg,F_low,lower = TRUE)

    Xbound_list[[i]] <- betatot[i]*exp((1/gammatot[i])*log(alphaavg) + (1/gammatot[i])*log((1/alphaavg)*Rgamma.inv(alphaavg,Fbound,lower = TRUE)))
  ...
  if (!is.null(dim(databystress))){
    ...
    plotout<-ggplot() +
      ...
      scale_y_continuous(trans = 'log10', limits = c(min(fcB), max(fcB)), breaks=Pticks, labels=Pticks1label) +
      xlab(xlabel1) +
    ...
    for(i in 1:length(databystress)){
      xlines[((i*2) - 1):(i*2)] <- betatot[i]*exp((1/gammatot[i])*log(alphaavg) + (1/gammatot[i])*log((1/alphaavg)*Rgamma.inv(alphaavg,c(0.001,0.999),lower = TRUE)))
      Flines[((i*2) - 1):(i*2)] <- Rgamma.inv(alphaavg,c(Fmin,Fmax),lower = TRUE)
      ...
      line_legend[((i*2) - 1):(i*2)] <- rep(paste(c("\U03B1 = ",num2str(outputpp1[[i]][[2]][1],2),", \U03B2 = ",num2str(outputpp1[[i]][[2]][2],2),", \U03B3 = ",num2str(outputpp1[[i]][[2]][3],2)),collapse = ""),2)
      if(length(databystress) > 1){ # multiple groups of data
        line_breaks[i] <- paste(c("\U03B1 = ",num2str(outputpp1[[i]][[2]][1],2),", \U03B2 = ",num2str(outputpp1[[i]][[2]][2],2),", \U03B3 = ",num2str(outputpp1[[i]][[2]][3],2)),collapse = "")
      } else{ # only one group
        line_breaks <- waiver()
      }
  ...
  if(is.null(MLE_i) == TRUE){
    ...
    cat("\n")
    cat(c("Generalized Gamma Least-Squares estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
    ...
    # Produce some output text that summarizes the results
    cat(c("Generalized Gamma Maximum Likelihood estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
