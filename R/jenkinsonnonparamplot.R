# Jenkinson (or Beard) Non-Parametric Plot
# Developed by Dr. Reuel Smith, 2020-2022

nonparamplot.beard <- function(xi, rc, relfcn, confbnd, xlabel) {
  if(missing(relfcn)) {
    relfcn<-"reliability"
  }
  ...
  if (missing(rc)){
    FRhH<-plotposit.beard(rankcalc(xi),xi)
    nonparamset<-plottable.nonparam(xi, NULL, FRhH, relfcn, alpha, xlabel)
  } else {
    FRhH<-plotposit.beard(rankcalc(xi,rc),xi, rc)
