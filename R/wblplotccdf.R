...
plot.reliability.wbl <- function(xi, rc = NULL, pp = "Blom", confid = 0.95, sided = "twosided", xlabel1 = "X"){
  library(pracma)
  library(mvtnorm)
  library(ggplot2)
  ...
  yccdf <- 1 - pweibull(xrange, params$wblparams[2], params$wblparams[1])
  ...
  yccdf_low <- exp(-exp(u_low))
  yccdf_high <- exp(-exp(u_high))

  # Form data frame
  df <- data.frame(X = xrange, YRel = yccdf, YRellow = yccdf_low, YRelhigh = yccdf_high, best_fit = rep("Fitted",1000))
  # return(list(df))
  df2 <- data.frame(Xpts = x_R[,1], Ypts = x_R[,3], nonparametric_data = rep("Failure Data",length(x_R[,1])))
  plotout<-ggplot() +
    geom_line(data=df, aes(X,YRel), colour = 'blue', size = 0.9, linetype = "dashed") +
    xlab(xlabel1) +
    ylab("Percent Survival")
  plotout <- plotout + geom_ribbon(data=df, aes(ymin=YRellow, ymax=YRelhigh, x=X), alpha=0.5, fill = "blue")
  plotout <- plotout + geom_point(data=df2, aes(Xpts,Ypts, shape = nonparametric_data), colour = 'red', size = 2.2)
  return(list(wbloutput = params,Reliabilityplot = plotout))
}
