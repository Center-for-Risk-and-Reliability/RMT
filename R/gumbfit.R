...
fit.gumb <- function(xi, rc = NULL, pp = "Blom", confid = 0.95, sided = "twosided"){
  library(pracma)
  ...
  est1 <- c(probplotparam.gumb(x_R[,1], x_R[,3])[[3]])
  # MLE estimate
  est2 <- distribution.MLEest(est1, "Gumbel", xi, rc, confid, sided)
  ...
  cat(c("Maximum-Likelihood estimates for Gumbel Fit.\n\n"),sep = "")
  if(sided == "twosided"){
  ...
  return(list(gumbparams = est2[[1]], CImu = est2[[3]][[1]], CIsigma = est2[[3]][[2]], varcovmat = est2[[2]], loglikelihood = est2[[4]], AIC = AIC, BIC = BIC))
}
