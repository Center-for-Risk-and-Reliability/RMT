...
probplot.loglogist <- function(data,pp="Blom",xlabel1="X",confid=0.95,
  ...
  if(is.null(MLE_i)==TRUE){ # Switch off for LSQ
    best_fit_label <- "Loglogistic (LSQ) Best-Fit"
  }
  if(is.null(MLE_i)==FALSE){ # Switch on for MLE
    best_fit_label <- "Loglogistic (MLE) Best-Fit"
  }
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      ttfc_list[[i]]<-probplotparam.loglogist(xiRFblock_list[[i]][,1],xiRFblock_list[[i]][,2],CDFrangesetting)
      ...
      if(is.null(MLE_i) == FALSE){ # MLE for all parameters μ and σ
        # Compute MLE
        ttfc_MLE_list[[i]] <- distribution.MLEest(c(ttfc_list[[i]][[3]]),"Loglogistic",ixirc_list[[i]][[2]],ixirc_list[[i]][[3]])
        # Recalculate x-points if MLE
        ttfc_list[[i]][[1]] <- exp((ttfc_MLE_list[[i]][[1]][1]) + (ttfc_MLE_list[[i]][[1]][2])*fcB)

        outputpp[[i*3-1]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 2, byrow = TRUE,dimnames = list(c("Log-logistic Parameters"),c("mu", "sigma")))
        outputpp1[[i]][[2]]<-matrix(ttfc_MLE_list[[i]][[1]], nrow = 1, ncol = 2, byrow = TRUE,dimnames = list(c("Log-logistic Parameters"),c("mu", "sigma")))
  ...
  for(i in 1:length(databystress)){
    ...
    Xbound_list[[i]] <- exp((ttfc_list[[i]][[3]][1]) + (ttfc_list[[i]][[3]][2])*FB_bound)
  ...
  if(is.null(MLE_i) == TRUE){
    ...
    cat(c("Loglogistic Least-Squares estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
    ...
    # Produce some output text that summarizes the results
    cat(c("Loglogistic Maximum Likelihood estimates\n\n"))
    if(length(3:length(databystress[[i]][1,])) == 1){
