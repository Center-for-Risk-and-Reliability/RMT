...
# Define UI ----
ui <- fluidPage(
  titlePanel("Life-Stress Bayes Modeling Toolkit"),

  sidebarLayout(
    sidebarPanel(
      helpText(
        "The life-stress Bayesian modeling toolkit takes new data and updates the
        parameters of a specified life-stress model with respect to a given life
        distribution.  To start, prepare your data in a CSV file with the new data
      ...
      helpText(
        "Select the life distribution model and the life-stress model that you will fit
        to the data."),
      # Input panel for Use Stress
      textInput(
        "priors", h4("Prior Distributions"), value = ""
      ),
      helpText(
        "Enter the values for your parameter priors here.  Enter using the given format
        that rStan uses for distributions.  Enter each distribution in quotes and separate them by commas.
        Example:''uniform(0,100)'',''normal(3,0.4)'',...  Enter the priors in the
        same order as your initial estimate is given based on the life model and life-stress
        model."),
      # Input panel for confidence
      numericInput(
        "conf", h4("Confidence Bounds (%)"), min = 1, max = 99.99, value = 95
      ),
      # Input panel for samples and burn-in count
      textInput(
        "samplesno", h4("Number of Samples"), value = "100000"
      ),
      textInput(
        "burnin", h4("Burn-in"), value = "5000"
      ),
      helpText(
        "Enter the number of samples for each chain of the Markov analysis.  Also enter
        the number of samples the analysis will throw away (burn-in) for each chain"),
      # Input panel for Use Stress
      textInput(
